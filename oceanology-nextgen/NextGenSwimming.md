...
- A character Blueprint with a **Character Movement Component** (the default Third Person template works well).
- Basic familiarity with **Blueprints**, **components**, and **collision** in Unreal Engine.

## Notes
- Swimming in Oceanology Next-Gen requires two components working together: **OceanBuoyancy** handles floating physics while **OceanSwimming** manages swim locomotion and transitions.
- The **Oceanology Water Volume** must have swimming enabled for characters to enter swim mode when overlapping the volume.
- **Water Interactor Meshes** are optional but recommended — they create realistic splash effects when the character's hands interact with the water surface.
- This guide uses the included **BP_ThirdPersonCharacter** as a reference. You can apply the same setup to any character Blueprint.
